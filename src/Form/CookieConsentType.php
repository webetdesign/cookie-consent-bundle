...
namespace WebEtDesign\CookieConsentBundle\Form;
...
class CookieConsentType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options) : void
    {
        foreach ($this->cookieCategories as $category) {
            $builder->add($category, ChoiceType::class, [
                ...
                'choices'  => [
                    ['wd_cookie_consent.yes' => 'true'],
                    ['wd_cookie_consent.no' => 'false'],
                ],
            ]);
        }
        ...
        if ($this->cookieConsentSimplified === false) {
            $builder->add('save', SubmitType::class, ['label' => 'wd_cookie_consent.save', 'attr' => ['class' => 'ch-cookie-consent__btn hide']]);
            $builder->add('use_only_functional_cookies', SubmitType::class, ['label' => 'wd_cookie_consent.use_only_functional_cookies', 'attr' => ['class' => 'ch-cookie-consent__btn']]);
            $builder->add('use_all_cookies', SubmitType::class, ['label' => 'wd_cookie_consent.use_all_cookies', 'attr' => ['class' => 'ch-cookie-consent__btn']]);

            $builder->addEventListener(FormEvents::PRE_SUBMIT, function (FormEvent $event) {
                $data = $event->getData();

                if (!array_key_exists('save', $event->getData())){
                    foreach ($this->cookieCategories as $category) {
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'translation_domain' => 'WDCookieConsentBundle',
